...
function SectionHeading({ children }: { children: React.ReactNode }) {
  return (
    <div className="mb-6 flex items-baseline gap-3">
      <p className="whitespace-nowrap text-lg font-semibold text-slate-400">
        {children}
      </p>
  ...
  return (
    <main className="mx-auto max-w-2xl px-6 pb-32 pt-14 sm:pt-20">
      <header className="mb-10">
        <h1 className="text-4xl font-bold text-[#1a1f25]">Mason Veilleux</h1>
      ...
      <p className="text-xl leading-relaxed text-slate-800">
        <strong className="font-semibold text-[#1f403c]">
          I design and build machine learning and AI pipelines for optimizing
          decisions under uncertainty.
        </strong>{" "}
        I care most about ones that are transparent, trustworthy, and give
        decision-makers the confidence they need.
      </p>

      <p className="mt-5 text-xl leading-relaxed text-slate-800">
        <strong className="font-semibold text-[#1f403c]">I am most excited about</strong>{" "}
        building and deploying statistical models to optimize decision-making
        alongside fostering a culture of trust and empowerment with data.
      </p>
        ...
        <div className="-mx-2">
          {workExperience.map((job) => (
            <div
              ...
              className="grid grid-cols-1 gap-1 rounded-md px-2 py-3.5 sm:grid-cols-[minmax(0,1.1fr)_minmax(0,1fr)] sm:items-baseline sm:gap-8"
            >
              <span className="text-xl font-medium text-[#1f403c]">{job.role}</span>
              <span className="text-lg text-slate-500 sm:text-right">{job.company}</span>
            </div>
          ))}
        </div>
      </section>
            ...
            {recentPosts.map((post) => (
              <li key={post.slug}>
                <Link
                  ...
                  className="group block rounded-md px-2 py-1 -mx-2 transition-colors hover:bg-black/[0.03]"
                >
                  <span className="text-xl font-medium text-[#1f403c] group-hover:underline underline-offset-4">
                    {post.title}
                  </span>
                  <p className="mt-1.5 text-lg leading-relaxed text-slate-500">
                    {post.excerpt}
                  </p>
                </Link>
              </li>
          ...
          {talks.map((talk) => (
            ...
            >
              <div className="flex flex-wrap items-baseline gap-x-2 gap-y-0.5">
                ...
                <span className="text-base text-slate-400">Slides (PDF)</span>
              </div>
              <p className="mt-1 text-base text-slate-400">{talk.event}</p>
      ...
      <section className="mt-14">
        <SectionHeading>Things that inspire me</SectionHeading>
        <ul className="space-y-5">
          {inspiredBy.map((item) => (
            <li key={item.title} className="flex gap-3 text-lg">
              <span className="mt-1.5 shrink-0 text-slate-300">—</span>
              <div>
                <a
                  href={item.url}
                  target="_blank"
                ...
                <p className="mt-1.5 text-slate-500">{item.note}</p>
              </div>
            </li>
          ))}
        </ul>
