...
export default function WritingsPage() {
  ...
  return (
    <main className="mx-auto max-w-2xl px-6 pb-24 pt-10 sm:pt-14">
      ...
      <div className="mt-8">
        <p className="text-sm font-semibold uppercase tracking-[0.4em] text-slate-400">Writings</p>
        ...
        {posts.map((post) => (
          ...
          >
            <div className="flex items-baseline gap-2">
              ...
              <p className="shrink-0 tabular-nums text-base text-slate-400">{formatDate(post.date)}</p>
            </div>
            <p className="mt-1.5 text-lg text-slate-500">{post.excerpt}</p>
            {post.tags && post.tags.length > 0 && (
              <div className="mt-2 flex flex-wrap gap-1.5">
                {post.tags.map((tag) => (
                  <span key={tag} className="text-xs font-semibold uppercase tracking-[0.15em] text-slate-400">
                    {tag}
                  </span>
                ))}
              </div>
